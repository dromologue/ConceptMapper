...
import type { GraphNode, Stream, NodeTypeConfig } from "../types/graph-ir";
...
export function Sidebar({
  nodes, streams, nodeTypeConfigs, activeStreams,
          ...
          <div className="sidebar-section-body sidebar-node-list">
            ...
            <div className="sidebar-node-scroll">
              {filteredNodes.map((n) => {
                ...
                const icon = typeConfig?.icon ?? typeConfig?.label?.[0] ?? n.node_type[0]?.toUpperCase() ?? "?";
                return (
                  <div
                    key={n.id}
                  ...
                  >
                    <span
                      className={`sidebar-node-indicator ${typeConfig?.shape === "rectangle" ? "concept" : ""}`}
                      style={{
                        backgroundColor: streams.find((s) => s.id === n.stream)?.color ?? "#666",
                      }}
                    />
                    <span className="sidebar-node-name">{n.name}</span>
                    <span className="sidebar-node-type">{icon}</span>
                  </div>
                );
              })}
            </div>
